package com.borda.rfid_reader_sdk
...
import com.borda.rfid_reader_sdk.utils.*
...
class ZebraConnectionHelper(
    ...
    fun findTheTag(tag: String) {
        viewModelScope.launch(Dispatchers.IO) {
            Log.d(LOG_TAG, "ssssssssssssssssssssssssssssssssssssssssssssss")
            try {
                if (reader != null && reader!!.isConnected) {
    ...
    @Synchronized
    fun findMultipleTag(tags: List<String>) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                if (reader != null && reader!!.isConnected) {
                    Log.d(LOG_TAG, "findMultipleTag called!")
                    ...
                    for (tag in tags) {
                        ...
                        // Optional: Add delay between tag processing
//                        delay(500) // adjust delay as needed
                ...
                e.printStackTrace()
                Log.d(LOG_TAG, "OperationFailureException ${e.vendorMessage}")
            }
        }
    }
